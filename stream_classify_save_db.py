...
class TwitterStreamer(StreamListener):
	...
	def on_data(self, data):
		try:			
			...
			ts = datetime.datetime.strftime(datetime.datetime.strptime(json.loads(data)['created_at'],'%a %b %d %H:%M:%S +0000 %Y') + datetime.timedelta(hours=7), '%Y-%m-%d %H:%M:%S')

			con = mysql.connector.connect(host='localhost', database='twitmacet', user='root', password='')
			cur = con.cursor()
			add_tweet = ("INSERT INTO tweets(date_time, raw_tweet, naive_bayes, svm, decision_tree, created_at) VALUES(%s, %s, %s, %s, %s, %s)")
			tweet_data = (ts, tweet, str(self.classifier.naive_bayes_classify(tweet)), str(self.classifier.svm_classify(tweet)), str(self.classifier.decision_tree_classify(tweet)), datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
